...
function Frame({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-full w-full flex-col rounded-2xl border border-foreground/10 bg-white/70 p-6 sm:p-7">
      {children}
    </div>
  );
}
...
function Caption({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="text-[11px] tracking-[0.09em] text-foreground/40 uppercase"
      style={headingFont}
    >
      {children}
    </div>
      ...
      <ul className="flex flex-col gap-2.5">
        {lines.map((line) => (
          <li key={line.label} className="flex items-baseline gap-3 text-[13px]">
            <span className="shrink-0 text-foreground/35 tabular-nums">{line.time}</span>
    ...
    <Frame>
      ...
      <div className="flex flex-wrap gap-1.5">
        {tools.map((tool) => (
          <span
            key={tool}
            className="rounded-full border border-foreground/10 px-2.5 py-1 text-[11px] text-foreground/60"
          >
            {tool}
          </span>
        ))}
      ...
      <dl className="flex flex-col gap-2.5">
        {specs.map((spec) => (
          <div key={spec.label} className="flex items-baseline gap-3">
            <dt className="w-[86px] shrink-0 text-[11px] tracking-[0.06em] text-foreground/40 uppercase">
              {spec.label}
            </dt>
      ...
      <ul className="mt-4 flex flex-col gap-3">
        {items.map((item) => (
          <li key={item} className="flex items-center gap-3 text-[13px] text-foreground">
            <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-foreground/25">
...
export function KnowledgeBaseVisual() {
  ...
  return (
    <Frame>
      ...
      <div className="flex items-center gap-2 rounded-lg border border-foreground bg-accent px-3 py-2.5 text-[12px] text-accent-foreground">
        ...
        {["Recherche intelligente (RAG)", "Droits d'accès par équipe", "Confidentialité garantie"].map(
          (line) => (
            <li key={line} className="flex items-baseline gap-3">
              <span className="h-[5px] w-[5px] shrink-0 rounded-full bg-foreground/35" />
